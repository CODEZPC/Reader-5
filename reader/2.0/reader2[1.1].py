...
lines = []
eng_char = "1234567890ABCDEFGHIJKLMNefghijklmOPQRSTUVWXYZabcdnopqrstuvwxyz!@#$%^&*()_+-={}[]|\\:\"';<>?,./`~ ·"
now = 0
full = 0
...
def openfile(event=None):
    global lines, full, file, title, now, status
    ...
    if file_path:
        ...
        for i in lines:
            ...
            if idxl == CONFIG["LINES_PER_PAGE"] + 1:
                now = 1
                update(lines, 1)
            if full % 20 == 0 and full > 0:
                status.configure(text=f"第{now}/{full}页 {round((now/full)*100,3)}%")
...
def update(txt, page):
    ...
    status.configure(text=f"第{page}/{full}页 {round((page/full)*100,3)}%")
